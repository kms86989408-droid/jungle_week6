...
int main()
{
	...
	printf("3: Remove values until the given value; / 3: 지정한 값이 나올 때까지 제거\n");
	printf("0: Quit: / 0: 종료\n");
	...
	while (c != 0)
	{
		...
		case 1:
			...
			break;
		case 2:
		    printf("Enter an integer value in stack to remove values until that value: / 해당 값이 나올 때까지 제거할 기준 값을 입력하세요: ");
			scanf("%d", &i);
			removeUntil(&s,i); // You need to code this function (이 함수는 직접 구현해야 합니다)
			printf("The resulting stack after removing values until the given value: / 지정한 값이 나올 때까지 제거한 결과 스택: ");
			printList(&(s.ll));
