...
int main()
{
	...
	printf("2: Move the largest stored value to the front of the list: / 2: 가장 큰 값을 맨 앞으로 이동\n");
	printf("0: Quit: / 0: 종료\n");
	...
	while (c != 0)
	{
		...
		case 1:
			...
			break;
		case 2:
			moveMaxToFront(&(ll.head));  // You need to code this function (이 함수는 직접 구현해야 합니다)
			printf("The resulting linked list after moving largest stored value to the front of the list is: / 가장 큰 값을 앞으로 이동한 결과 연결 리스트: ");
			printList(&ll);
			removeAllItems(&ll);
			break;
		case 0:
